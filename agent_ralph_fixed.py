...
def get_latest_email(service):
    results = service.users().messages().list(
        userId="me",
        q=f"from:{RECIPIENT_EMAIL} to:{SENDER_EMAIL} is:unread",
        maxResults=1
    ).execute()
...
def log_to_notion(task_title, task_body):
    ...
    payload = {
        ...
        "properties": {
            ...
            "Summary": {
                "rich_text": [{"text": {"content": task_body}}]
            },
            "Status": {
...
def main():
    ...
    try:
        ...
        notion_logged = log_to_notion(title, content)
        send_email(
            gmail_service,
            RECIPIENT_EMAIL,
